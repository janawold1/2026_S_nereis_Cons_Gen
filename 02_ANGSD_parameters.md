...
It is important to have the ancestral state for estimates of the site frequency spectrum for theta-based statistics and identifying derived alleles for load estimates. We downloaded short reads sequenced by the [B10K](https://b10k.genomics.cn/species.html) consortium on NCBI for large-billed tern ([*Phaetusa simplex*](https://www.ncbi.nlm.nih.gov/nuccore/VZZW00000000.1) NCBI BioSample: SAMN12253896), black skimmer ([*Rhynchops niger*](https://www.ncbi.nlm.nih.gov/nuccore/VXBH00000000.1) NCBI BioSample: SAMN12253840), and common tern ([*Sterna hirundo*](https://www.ncbi.nlm.nih.gov/nuccore/WNMW00000000.1) NCBI BioSample: SAMN12369541) to polarise sites for fairy terns. For kakī, we leveraged resequence data generated by [Galla et al 2019](https://doi.org/10.3390/genes10010009) for the avocet (*Recurvirostra avosetta*) and pied stilt (*Himantopus himantopus*). This ([brief discussion](https://www.biostars.org/p/298013/)) outlines the benefit of using this approach for ANGSD-base analyses.  

For all outgroups, the reads were trimmed, aligned, and duplicates marked in the same manner as the fairy tern and kakī population short-reads. After filtering for autosomal scaffolds and evaluating the depth and coverage for each of the outgroups, BAMs were then randomly subsampled until all were of similar depth before merging into a single BAM for the fairy tern outgroups and another for the kakī outgroups. The large-billed tern and the pied stilt were not downsampled because they were the samples with the lowest coverage. Below is an example of how the fairy tern outgroups were subsampled and merged.  
```
samtools view -O BAM -o bams/black_skimmer_autosomes_nodup_0.77subsample.bam -s 0.77 bams/black_skimmer_autosomes_nodup.bam
samtools view -O BAM -o bams/common_tern_subsamp.bam -s 0.2 bams/common_tern_autosomes_nodup.bam

samtools merge -@4 -o tern_merged.bam bams/common_tern_subsamp.bam bams/large-billed_tern_autosomes_nodup.bam bams/black_skimmer_autosomes_nodup_0.77subsample.bam
```
An ancestral reference for ANGSD was then constructed.  
```
